...
std::vector<image_info> get_image_listing(
	...
	const std::string& imageFileName = "image.nii.gz",
	const std::string& labelFileName = "label.nii.gz"
)
{
	...
	{
		std::cerr << dataFolder << "not exist!!!" << std::endl;
		return results;
	}
		...
		it != boost::filesystem::directory_iterator(); ++it)
	{
		...
		image_info.image_filename = it->path().string() + "/" + imageFileName;
		image_info.label_filename = it->path().string() + "/" + labelFileName;

		// check if both image file and label file exists
		if (boost::filesystem::exists(image_info.image_filename) &&
			boost::filesystem::exists(image_info.label_filename))
		{
			results.push_back(image_info);
		}
		else
		{
			std::cerr << dataFolder << " does not contain valid files, i.e. " << imageFileName << " and " << labelFileName << std::endl;
		}
	}
...
std::vector<image_info> get_train_image_listing(const std::string& dataFolder)
{
	return get_image_listing(dataFolder + "/training");
}

// Read the list of testing image files
std::vector<image_info> get_test_image_listing(const std::string& dataFolder)
{
...
int main()
{
	boost::filesystem::path dataFolder("D:/Projects/dlib_resnet/data_crop");
	boost::filesystem::path trainerState("D:/Projects/dlib_resnet/trainer_state.dat");
	...
	trainer.set_synchronization_file(trainerState.string(), std::chrono::minutes(10));
	...
	const auto trainListing = get_train_image_listing(dataFolder.string());
